...
let time = 5 * 60;
        ...
        const countdown = setInterval(() => {
            ...
            if (time <= 0) {
                ...
                alert("Time's up!");
                window.location.href = 'sql-result.html';
            }
            time--;
        }, 1000);
...
function checkAnswer(selectedOption) {
    ...
    setTimeout(nextQuestion, 1000); // Delay to let the user see the color change
}
...
function nextQuestion() {
    if (currentQuestionIndex < questions.length - 1) {
        ...
        window.location.href = 'sql-result.html'; // Redirect to results page
    }
}
